...
export function CreatePurchaseOrderTab({
  ...
  const [status, setStatus] = useState<'Draft' | 'Approved' | 'Billed' | 'Declined'>('Draft')
  ...
  const addLineItem = () => {
    const defaultAcc = accounts.find(a => a.class_type === 'Expense')?.id || accounts[0]?.id || ''
  ...
  // Save / Approve PO
  const handleSavePO = async (targetStatus?: 'Draft' | 'Approved' | 'Billed' | 'Declined') => {
  ...
  const handlePrintPDF = async () => {
    ...
    try {
      ...
      const url = `http://localhost:8000/api/purchase-orders/download-pdf/?_t=${Date.now()}`
          ...
          <div>
            ...
            {editingPoId && (
              <span className={`inline-block text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider mt-1 border ${
                status === 'Approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-100/30' :
        ...
        <div className="flex items-center space-x-2.5 flex-wrap sm:justify-end gap-2">
          {editingPoId ? (
            <>
              ...
              {/* Symmetrical Three Vertical Dots Dropdown Menu */}
            ...
            <>
              {/* Creating new purchase order actions */}
              <button
                onClick={() => {
                  if (setEditingPoId) setEditingPoId(null)
                  setActiveTab('PurchaseOrders')
