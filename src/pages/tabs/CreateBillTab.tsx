...
export function CreateBillTab({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const loadedBanks = loadedAccounts.filter(a => a.type === 'Bank')
            ...
            <div className="absolute right-0 top-full mt-1.5 w-48 bg-white border border-slate-200 rounded-[3px] shadow-xl z-50 p-1.5 font-normal text-xs text-slate-700 animate-scaleIn">
              <button
                onClick={async () => {
                  ...
                  const id = await handleSaveBill('Awaiting Payment', false, true)
                  if (id) {
  ...
  return (
    <div className="space-y-6 text-left font-sans w-full px-4 sm:px-6 lg:px-8 animate-fadeIn pb-12">
            ...
            <>
              ...
              {status === 'Paid' && (
                <>
                  <button
                    onClick={handlePrintPDF}
                    className="bg-white hover:bg-slate-50 text-slate-700 hover:text-[#0F5B38] border border-slate-200 hover:border-slate-300 font-medium text-xs px-4.5 py-2 rounded-[3px] shadow-sm transition duration-200 cursor-pointer h-[38px] flex items-center justify-center space-x-1.5 animate-fadeIn"
                  >
                    <span>Print PDF</span>
                  </button>
                  <button
                    onClick={() => setIsShareModalOpen(true)}
                    className="bg-[#0F5B38] hover:brightness-105 text-white font-medium text-xs px-4.5 py-2 rounded-[3px] shadow-md transition duration-200 cursor-pointer h-[38px] flex items-center justify-center space-x-1.5 animate-fadeIn"
                  >
                    <span>Share</span>
                  </button>
                </>
      ...
      {isShareModalOpen && editingBillId && (
        <ShareModal
          docType="bill"
          docId={editingBillId}
